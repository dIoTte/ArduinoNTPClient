...
uint32_t NetworkTimeProtocolClient::getCurrentNTPTime() {
	...
	if(_UDP->begin(_localPort))	{
		while (currentNTPTime == 0 && _ntpServers[serverIndex] != 0) { 		// Iterates over virtual clusters [0-3].pool.ntp.org until success or list exhausted
			...
			Serial.println(_ntpServers[serverIndex]);
			WiFi.hostByName(_ntpServers[serverIndex], destinationIP);		// Destination IP is used later to ensure that the NTP request and response match
			if(this->sendRequestNTPPacket(destinationIP)) {					// Send an NTP packet to time server
				currentNTPTime = this->readResponseNTPPacket(destinationIP);// Read NTP response packet from time server
			}
			serverIndex++;
		}
		if(currentNTPTime == 0) {
...
uint32_t NetworkTimeProtocolClient::getCurrentEpochTime() {
...
uint8_t NetworkTimeProtocolClient::sendRequestNTPPacket(IPAddress& destinationIP) {
	...
	_UDP->beginPacket(destinationIP, _remotePort);
	_UDP->write(_packetBuffer, NTP_PACKET_SIZE);
	return _UDP->endPacket();
}

uint32_t NetworkTimeProtocolClient::readResponseNTPPacket(IPAddress& destinationIP) {
	...
	while (timeoutIndex++ < 100 && packetBytes == 0) { // Timeout after ~1 sec if no NTP response packet is received
		...
		if (packetBytes > 0 && _UDP->remoteIP() != destinationIP) { // UDP hole punching technique used by NAT could potentially create a request/response mismatch
